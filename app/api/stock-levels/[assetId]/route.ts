...
import { StockLevelService } from "@/services/stock/stockLevelService";

/**
 * GET /api/stock-level/[assetId]
 * Fetch a specific assetId stock level for bulk stock
 */
export const GET = withKeeperAuth(async (req: Request) => {
  ...
  try {
    const asset = await StockLevelService.getBulkAssetStockLevel(assetId)
    if (!asset) {
    ...
    return NextResponse.json(
      { error: error.message || "Failed to fetch asset stock level" },
      { status: 500 },
    );
  }
});
