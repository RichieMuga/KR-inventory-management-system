...
export function Header() {
  ...
  const navLinks = [
    ...
    { href: "/", label: "Assets" }, // Root path for Assets
  ...
  return (
    <header className="sticky top-0 z-40 flex h-16 items-center justify-between gap-4 border-b bg-background px-4 md:px-6">
      <Link href="/" className="flex items-center gap-2 text-lg font-semibold md:text-base">
        <Image src="/kr_logo.png" alt="Kenya Railways Logo" width={40} height={40} className="h-8 w-auto" />
        <span className="sr-only">Kenya Railways Inventory</span>
        <span className="hidden md:inline-block text-kr-maroon-dark font-bold">Inventory</span>
      </Link>
        ...
        {navLinks.map((link) => (
          <Link
            ...
            className={cn(
              ...
              pathname === link.href ? "text-kr-orange font-semibold" : "text-muted-foreground",
            )}
          >
            {link.label}
          </Link>
