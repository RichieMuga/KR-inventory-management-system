...
export function AssetListView() {
  ...
  const handleCreateAsset = (newAssetData: Omit<Asset, "id">) => {
    const newId = (Math.max(...assets.map((a) => Number.parseInt(a.id))) + 1).toString()
    const newAsset: Asset = {
      id: newId,
      ...newAssetData,
      serialNumber: newAssetData.isBulk ? undefined : newAssetData.serialNumber, // Ensure serialNumber is undefined for bulk
      quantity: newAssetData.isBulk ? newAssetData.quantity : 1, // Ensure quantity is 1 for unique if not specified
    }
    ...
    <div className="flex flex-col gap-4 p-4 md:p-6">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-4 w-full md:w-auto">
          ...
          <CreateAssetForm onCreateAsset={handleCreateAsset} />
        </div>
      </div>
